...
//   const category = params?.category || "default-category"; // Default value in case it's undefined
...
// ✅ Fetch product data with proper variable handling
async function getData(category: string): Promise<SimplifiedProduct[]> {
  const query = `*[_type == "Product" && category->name == $category]{
    _id,
    "imageUrl": images[0].asset->url,
...
// ✅ Correct Next.js Async Component Handling
export default async function CategoryPage({ params }: { params: { category: string } }) {
  const category = decodeURIComponent(params.category); // Handle URL encoded category names
  const data: SimplifiedProduct[] = await getData(category);
